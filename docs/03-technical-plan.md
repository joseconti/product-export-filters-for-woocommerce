...
| Path | State | Purpose |
|---|---|---|
| `export-filters-for-woocommerce.php` | [A] — Sprint 1 | Plugin bootstrap: header, WooCommerce-active guard, loads the includes, registers the `plugins_loaded` init hook |
| `includes/class-efwc-date-filter.php` | [A] — Sprint 1 | The `EFWC_Date_Filter` class: render, enqueue, query_args, clean_date (ported from `docs/filtros-exportador-woocommerce.md` §6, renamed to project conventions) |
| `languages/export-filters-for-woocommerce.pot` | [G] — generated by `wp i18n make-pot` at Sprint 1 / Phase 6 | Translation template, English source strings |
| `languages/export-filters-for-woocommerce-es_ES.po` / `.mo` | [A] — Sprint 1 (v1 ships es_ES per Phase 1 i18n decision) | Spanish translation |
| `readme.txt` | [A] — Phase 7 | wordpress.org listing (description, changelog, "Tested up to", FAQ) |
| `uninstall.php` | [A] — Sprint 1 | No-op beyond WordPress's own plugin removal — no options/tables to clean, but the file is present per WP convention and documents that explicitly in a comment |
| `tests/bootstrap.php` | [A] — Phase 5 scaffold | WP core PHPUnit test-suite bootstrap |
| `tests/unit/test-clean-date.php` | [A] — Sprint 1 (test-first, pure-logic) | Unit tests for `clean_date()` (AC-07) |
| `tests/unit/test-query-args.php` | [A] — Sprint 1 (test-first, pure-logic) | Unit tests for the args-building logic (AC-03–06, AC-11), with WordPress functions mocked/stubbed via the WP test suite |
| `tests/e2e/export-date-filter.spec.js` | [A] — Sprint 1 | Playwright end-to-end test driving the real `Products > Export` screen against the wp-env playground (AC-01, AC-02, AC-08, AC-09, AC-10, AC-12) |
| `.wp-env.json` | [A] — Phase 5 scaffold | wp-env playground definition (WordPress + WooCommerce + seed plugin) |
| `tests/seed/seed-products.php` | [A] — Phase 5 scaffold | WP-CLI-runnable script seeding synthetic products across a spread of `post_date`/`post_modified` values, statuses (publish/draft/future), and a catalog large enough to force a multi-batch export (AC-08) |
| `.distignore` | [A] — Phase 7 | Excludes `tests/`, `.wp-env.json`, dev tooling from the wordpress.org release ZIP |
| `docs/api/hooks-and-filters.md` | [A] — Phase 6 | Documents the plugin's own extension points, if any are added beyond consuming WooCommerce's (none planned for v1 — reserved for the Later roadmap) |
