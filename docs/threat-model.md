...
|---|---|---|
| An unauthenticated or under-privileged request reaches the date-filter logic and exfiltrates product data outside their permission | The plugin's render callback only ever fires as part of WooCommerce's own `Products > Export` screen, which WooCommerce itself gates behind `current_user_can( 'edit_products' ) && current_user_can( 'export' )` before the screen renders at all. The plugin performs no capability check of its own because it never runs a code path WooCommerce hasn't already gated. | `IN PLACE` — inherited from WooCommerce core's own screen gating (verified by reading `class-wc-admin-exporters.php`'s `product_exporter_page_permissions()`, not by an independent low-privilege-user test of this plugin's own — none was written; a dedicated permission-denied Playwright test is a reasonable Later addition, not required for v1 since the plugin adds no gate of its own to fail) |
| CSRF / a forged request triggers a filtered export the admin never intended | The query-args filter verifies `$_POST['security']` against the `wc-product-export` nonce (`wp_verify_nonce()`) before trusting any of the plugin's own POST fields; on failure it returns `$args` unmodified rather than trusting attacker-controlled input | `IN PLACE` — verified by AC-11 unit tests (`tests/unit/test-query-args.php`, real, passing) |
| The filter fires outside a genuine AJAX export context (another plugin, a cron job, a REST call reusing the same `apply_filters` hook name) and trusts stray `$_POST` data | Guarded by `wp_doing_ajax()` plus an explicit `empty( $_POST['form'] )` check before any parsing happens; fails safe to `$args` unchanged | `IN PLACE` — same AC-11 unit tests |
| Malformed or malicious date input (SQL-injection attempt, oversized string, non-date garbage) reaches the database query | Every date value is validated with a strict `^\d{4}-\d{2}-\d{2}$` regex AND `checkdate()` before ever being concatenated into the WooCommerce-native operator string (`>=`, `<=`, `...`); WooCommerce's own `parse_date_for_wp_query()` then turns that into parameterized `WP_Query`/`$wpdb` arguments — the plugin never builds raw SQL itself | `IN PLACE` — verified by AC-07 unit tests and a dedicated `test_rejects_sql_injection_shaped_input` unit test, all real and passing (`tests/unit/test-clean-date.php`) |
| Reflected/stored XSS via the rendered `<select>`/`<input>` markup or their values | All output uses `esc_attr()`/`esc_html__()`; the date inputs are native `<input type="date">` with no free-text echo of user input anywhere in the rendered HTML | `IN PLACE` — verified by `phpcs --standard=WordPress` (which flags unescaped output; 0 errors on this file) |
| Another plugin/theme calls `woocommerce_product_export_product_query_args` with attacker-shaped `$args` before this plugin's callback runs, and this plugin blindly trusts `$args[$field]` already being set | The plugin only ever *writes* `$args[$field]`, never reads a pre-existing value from it — no trust placed in the incoming `$args` array's date-related keys | `IN PLACE` — verified by code inspection (`includes/class-efwc-date-filter.php`'s `query_args()` never reads `$args[...]` before writing it) rather than a dedicated hostile-plugin simulation test |
